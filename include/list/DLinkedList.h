...
void DLinkedList<T>::add(T e)
{
    ...
    Node *newNode = new Node(e, nullptr, this->tail);
...
{
    // add element e at index position
    // done
    ...
    if (index == 0){
        Node *newNode = new Node(e, this->head, nullptr);
    ...
    Node *newNode = new Node(e, p, p->prev);
    p->prev->next = newNode;
    p->prev = newNode;
    this->count++;
...
int DLinkedList<T>::indexOf(T item)
{
    ...
    for (int i = 0; i < count; i++){
        if (equals(p->data, item, this->itemEqual)){
            return i;
        }
        p = p->next;
    }
